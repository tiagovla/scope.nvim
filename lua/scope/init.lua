...
function M._setup_legacy()
    ...
    augroup ScopeAU
        ...
        " autocmd SessionLoadPost * lua require("scope.session").load_state()
    augroup END
    ]])
end
...
function M._setup()
    ...
    -- api.nvim_create_autocmd("SessionLoadPost", { group = group, callback = session.load_state })
    api.nvim_create_user_command("ScopeList", core.print_summary, {}) --TODO: improve this
    api.nvim_create_user_command("ScopeSaveState", session.save_state, {})
    api.nvim_create_user_command("ScopeLoadState", session.load_state, {})
end
